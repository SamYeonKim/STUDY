...
* 빛이 존재하는 씬에서 텍스처 이미지는 오브젝트의 모든 diffuse 컬러를 나타냄
* 2D 텍스처를 3D 오브젝트에 매핑
* `Albedo map`라고도 함
...
out vec3 FragPos;           // object의 월드 좌표
out vec3 Normal;            // 노말 벡터
...
void main() {
    // ambient
    // ambient 컬러는 대부분의 경우 diffuse 컬러와 동일
...
int main() {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    // 같은 VBO를 사용하므로 stride 값은 8 * sizeof(float)
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
