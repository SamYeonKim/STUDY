...
float ShadowCalculation(vec3 fragPos) {
    // fragment와 빛 사이의 벡터를 구함
    vec3 fragToLight = fragPos - lightPos;
    // 깊이 큐브 맵에서 깊이 값 가져옴
    // float closestDepth = texture(depthMap, fragToLight).r;
    // 깊이값의 범위가 [0,1] 이므로 far_plane값을 곱함
    // closestDepth *= far_plane;
    // fragment와 빛 사이의 벡터의 길이를 구함(== 깊이)
    ...
    // 뷰어까지의 오프셋 반경을 늘릴수 있음
    // 그림자가 멀리 떨어져 있으면 부드럽게 가까이 있으면 날카롭게 표현됨
