# glTF-sample-viewer metallic-roughness.frag 분석

* `glTF 파일` : 크로노스 그룹에서 발표한 3차원 장면과 모델을 표현하는 파일 포맷으로 JSON 표준에 기반하고 있음
* glTF 파일 형식 viewer 코드 중 metallic-roughness 부분에 관한 fragment shader
...
#ifdef USE_PUNCTUAL
    // apply~::getPointShade() 안에서 BRDF 값 계산
