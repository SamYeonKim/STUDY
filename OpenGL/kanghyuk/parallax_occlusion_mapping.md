...
* `Parallax Occlusion Mapping` 을 사용하여 텍스쳐 표면의 디테일과 깊이감을 줘보자.

* `Parallax Occlusion Mapping` 은 `Steep Parallax Mapping` 에서 한단계 더 나아간 버전이다.
* `Steep Parallax Mapping` 을 통해 구한 값과 직전의 값을 선형적으로 보간한다.
* 아래의 그림에서 `T_0` 의 깊이값은 `T_P`가 된다.

![](image/occlusion1.png)

# Result

![](image/parallax_occlusion.png)

# Shader Codes

## Vertex Shader Codes

* `Parallax Mapping` 과 동일한 코드이다.
...
vec2 ParallaxMapping(vec2 texCoords, vec3 viewDir) { 
    ...
    // cosine 값으로 interpolating 한다. (바라보는 방향이 텍스쳐의 수직에 가까우면 많은 레이어로 계산)
    ...
    // linear interpolation을 위해 depth 값을 가져온다.
    float afterDepth  = currentDepthMapValue - currentLayerDepth;
    float beforeDepth = texture(depthMap, prevTexCoords).r - currentLayerDepth + layerDepth;
 
    // interpolation of texture coordinates
    float weight = afterDepth / (afterDepth - beforeDepth); // 비율
