...
* `Steep Parallax Mapping` 을 사용하여 텍스쳐 표면의 디테일과 깊이감을 줘보자.

* `Steep Parallax Mapping`은 `depth map` 을 여러개의 레이어로 나누어 현재 점에서 가장 비슷한 깊이를 찾는 기법이다.
* 아래의 그림에서 `T_0` 의 깊이값은 `T_3`의 값인 0.6이 된다.
* 근사값이기 때문에 약간의 계단현상이 보인다.
  * 레이어 수를 늘리면 해결되지만, 연산량이 늘어난다.

![](image/steep1.png)

# Result

![](image/steep_parallax.png)
